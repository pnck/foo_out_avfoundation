...
//
//  The Swift controller is resolved by its @objc runtime name (VSurroundPreferencesViewController) rather
...
namespace
{
    ...
    constexpr GUID guid_preferences_vsurround = {
        0x2C7A1E94, 0x5B3D, 0x4F02, {0x9A, 0x1C, 0x77, 0xE0, 0x33, 0xAB, 0xCD, 0x12}
    };

    class preferences_page_vsurround : public preferences_page
    {
    public:
        service_ptr instantiate() override {
            // Swift class, registered with the ObjC runtime via @objc(VSurroundPreferencesViewController).
            Class vcClass = NSClassFromString(@"VSurroundPreferencesViewController");
            NSViewController *vc = [[vcClass alloc] init];
            return fb2k::wrapNSObject(vc);
        }
        const char *get_name() override { return "AVFoundation Output"; }
        GUID get_guid() override { return guid_preferences_vsurround; }
        // Sit under Playback > Output (where an output component belongs), not Tools.
        GUID get_parent_guid() override { return guid_output; }
    };

    FB2K_SERVICE_FACTORY(preferences_page_vsurround);
} // namespace
